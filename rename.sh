...
echo "$now Starting $filename.ts"

if [[ $1 =~ News ]]; then
	#echo "regex matched"
	echo "$now Not converting $1"
	/usr/bin/comskip $1
else
...
########/bin/sed -i "s/$filename.ts/$filename.mkv/g" /home/hts/.hts/tvheadend/dvr/log/*
	mv "$directory/$filename.ts.mkv" "$directory/$filename.mkv" 
	/usr/bin/comskip $directory/$filename.mkv
	#/bin/ln -s $1.mkv $1
fi
